...
public class ArmALU extends InstanceFactory {
    int xp[], yp[];
	...
	int height = 100;
		...
		new Port(interp(xp[3],xp[4],0.5), interp(yp[3],yp[4],0.5), Port.INPUT, 32), // B
                new Port(interp(xp[0],xp[1],0.33333), interp(yp[0],yp[1],0.33333), Port.INPUT, 2), // OP
		new Port(interp(xp[1],xp[2],0.25), interp(yp[1],yp[2],0.25), Port.OUTPUT, 4), // ST
    ...
    public void propagate(InstanceState state) {
        ...
        Value valueST = Value.createKnown(BitWidth.create(4), ans == 0 ? 1 : 0);
        state.setPort(C, valueC, 32);
        state.setPort(ST, valueST, 2);
    }
	...
	g.dispose();
        ...
        painter.drawPort(ST, "ST", Direction.WEST);
